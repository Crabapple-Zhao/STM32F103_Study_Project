...
//
// 修改：修复 exitAnimation/blur 中 bufferLen uint8_t 溢出 bug
//
...
inline void Animation::exitAnimation() {
  ...
  static uint32_t bufferLen = HAL::getBufferTileHeight() * HAL::getBufferTileWidth();  // 修复: 去掉 8* (tileHeight 已是像素/8)
...
inline void Animation::blur() {
  static uint32_t bufferLen = HAL::getBufferTileHeight() * HAL::getBufferTileWidth();  // 修复: 去掉 8*
