...
public:
  ...
  virtual void _drawEnglish(float _x, float _y, const char *_text) {}
...
public:
  ...
  //读取指定按键的当前动作（RELEASE/CLICK/PRESS），供 Launcher 等上层使用
  static key::keyAction getKeyAction(key::KEY_INDEX _keyIndex) { return get()->key[_keyIndex]; }

  //清空所有按键动作标记，处理完一次后调用
  static void clearKeyAction() {
    for (uint8_t i = 0; i < key::KEY_NUM; i++) get()->key[i] = key::RELEASE;
  }
